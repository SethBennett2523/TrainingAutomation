...
class EarlyStopping:
    ...
    def __init__(
        ...
        if self.verbose:
            self.logger.info(f"Early stopping initialized with patience={patience}, "
                            f"min_delta={min_delta}, metric={metric_name}")
    
    def check(self, epoch: int, metrics: Dict[str, float], model_path: str = None) -> bool:
        """
        ...
        if self.best_score is None:
            self._update_best(current_score, epoch, model_path)
            return False
